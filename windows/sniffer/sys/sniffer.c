...
#define WPP_CONTROL_GUIDS \
    WPP_DEFINE_CONTROL_GUID(SnifferMonitor,(dd65554d, 9925, 49d1, 83b6, 46125feb4207),  \
...
MonitorCoUnregisterCallouts(void)
{
   ...
   status = MonitorCoUnregisterCallout(&SNIFFER_SAMPLE_FLOW_ESTABLISHED_CALLOUT_V4);

   if (NT_SUCCESS(status))
   {
      status = MonitorCoUnregisterCallout(&SNIFFER_SAMPLE_STREAM_CALLOUT_V4);
   }

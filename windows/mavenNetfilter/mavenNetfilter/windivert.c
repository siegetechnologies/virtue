...
#ifdef WINDIVERT_DEBUG
#define PRINT_DBG wprintf
#else
#define PRINT_DBG(args)
#endif

#define WINDIVERTEXPORT
#include "..\mavenWebfilter\mavenWebfilter\windivert.h"
#include "..\mavenWebfilter\mavenWebfilter\windivert_device.h"
>>>>>>> mavenWindows
...
WinDivertDriverInstallReTry:
    ...
    if (service != NULL)
    {
		PRINT_DBG(L" WinDivertDriverInstall already exists\n");
		goto WinDivertDriverInstallExit;
...
WinDivertDriverInstallExit:
    ...
    if (service != NULL)
    {
        ...
        if (!StartService(service, 0, NULL))
        {
            ...
            {
				PRINT_DBG(L" WinDivertDriverInstall already running\n");
				SetLastError(0);
            }
            else
            {
                // Failed to start service; clean-up:
				PRINT_DBG(L" WinDivertDriverInstall failed to start service\n");
    ...
    INT16 priority, UINT64 flags)
{
	...
	PRINT_DBG(L" WinDivertOpen Compile the filter\n");
    // Compile the filter:
	...
	PRINT_DBG(L" WinDivertOpen Attempt to open the WinDivert device\n");
	...
	PRINT_DBG(L" WinDivertOpen Open failed because the device isn't installed\n");
       ...
       // Open failed because the device isn't installed; install it now.
        ...
        if (handle == INVALID_HANDLE_VALUE)
        {
	PRINT_DBG(L" WinDivertOpen INVALID_HANDLE_VALUE\n");
            return INVALID_HANDLE_VALUE;
        }
    }
    ...
    if (layer != WINDIVERT_LAYER_DEFAULT)
    {
        ...
        {
 	PRINT_DBG(L" WinDivertOpen bad layer \n");
           CloseHandle(handle);
            return INVALID_HANDLE_VALUE;
        }
    }
    ...
    if (flags != 0)
    {
        ...
        {
 	PRINT_DBG(L" WinDivertOpen can't set flags \n");
           CloseHandle(handle);
            return INVALID_HANDLE_VALUE;
        }
    }
    ...
    if (priority32 != WINDIVERT_PRIORITY_DEFAULT)
    {
        ...
        {
 	PRINT_DBG(L" WinDivertOpen can't set priority \n");
            CloseHandle(handle);
            return INVALID_HANDLE_VALUE;
        }
    }
    ...
    {
  	PRINT_DBG(L" WinDivertOpen can't start filter \n");
       CloseHandle(handle);
        return INVALID_HANDLE_VALUE;
    }
...
* Receive a metrics.
*/
extern BOOL WinDivertGetMetrics(HANDLE handle, PVOID pMetrics, UINT metricsLen)
{
	BOOL rtnVal;
	PRINT_DBG(L"Recv");
    ...
    LPOVERLAPPED overlapped)
{
	...
	if (flags != 0)
    {
		PRINT_DBG(L"ERROR_INVALID_PARAMETER ");
		SetLastError(ERROR_INVALID_PARAMETER);
        return FALSE;
    }
    if (overlapped == NULL)

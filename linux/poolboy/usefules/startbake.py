...
try:
	...
	print( "Usage: {} bakeID imageurl <url>".format(sys.argv[0]))
	sys.exit(0)
...
with open(str(Path.home()) + '/.ssh/id_rsa.pub') as f:
	start = json.loads(api('/bake/start', {"bakeID":bakeID, "srcurl":imageurl, "dsturl":imageurl + "_baked", 'info':{'bakeID':bakeID, 'no':True}, 'vconf':vconf}))
pprint.pprint(start)

...
pc('create an app for blender', bc.HEADER)
goodrole = json.loads(api('/application/create', {"userToken":admintok, "applicationID":"blender", "install":"blender wget curl unzip tar", "launchCmd":"blender", "icon":"https://download.blender.org/institute/BlenderDesktopLogo.png", "type":"linux"}))
...
pc('create a role for blender', bc.HEADER)
goodrole = json.loads(api('/role/create', {"userToken":admintok, "roleID":"blender", "applicationIDs":["xterm", "blender"]}))
pprint.pprint(goodrole)

pc('ok, admin will add rolo', bc.HEADER)
badv = json.loads(api('/user/role/authorize', {"userToken":admintok, "username":username, "roleID":"blender"}))
pprint.pprint(badv)
...
while True:
	badv = json.loads(api('/role/get', {"userToken":admintok, "roleID":"blender"}))
	pprint.pprint(badv)
...
badv = json.loads(api('/virtue/create', {"userToken":admintok, "roleID":"blender", "userID":username}))
pprint.pprint(badv)

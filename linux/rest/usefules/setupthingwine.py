...
pc('create an app for wine', bc.HEADER)
goodrole = json.loads(api('/application/create', {"userToken":admintok,
	"applicationID":"wine",
	"install":"wine-development",
	"launchCmd":"winecfg",
	"icon":"https://upload.wikimedia.org/wikipedia/commons/thumb/b/be/Wine_logo.svg/480px-Wine_logo.svg.png",
	"postInstall":"dpkg --add-architecture i386 && apt-get update -y && apt-get install -y wine32-development",
	"type":"linux",
	}))
...
pc('create a role for wine', bc.HEADER)
goodrole = json.loads(api('/role/create', {"userToken":admintok, "roleID":"wine", "applicationIDs":["xterm", "wine"]}))
pprint.pprint(goodrole)

pc('ok, admin will add rolo', bc.HEADER)
badv = json.loads(api('/user/role/authorize', {"userToken":admintok, "username":username, "roleID":"wine"}))
pprint.pprint(badv)
...
while True:
	badv = json.loads(api('/role/get', {"userToken":admintok, "roleID":"wine"}))
	pprint.pprint(badv)
...
badv = json.loads(api('/virtue/create', {"userToken":admintok, "roleID":"wine", "userID":username}))
pprint.pprint(badv)

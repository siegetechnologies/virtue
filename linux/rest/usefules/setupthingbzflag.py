...
pc('create an app for bzflag', bc.HEADER)
goodrole = json.loads(api('/application/create', {"userToken":admintok, "applicationID":"bzflag", "install":"bzflag", "launchCmd":"bzflag", "icon":"https://upload.wikimedia.org/wikipedia/commons/4/46/MacBZFlag.png", "type":"linux"}))
...
pc('create a role for bzflag', bc.HEADER)
goodrole = json.loads(api('/role/create', {"userToken":admintok, "roleID":"bzflag", "applicationIDs":["xterm", "bzflag"]}))
pprint.pprint(goodrole)

pc('ok, admin will add rolo', bc.HEADER)
badv = json.loads(api('/user/role/authorize', {"userToken":admintok, "username":username, "roleID":"bzflag"}))
pprint.pprint(badv)
...
while True:
	badv = json.loads(api('/role/get', {"userToken":admintok, "roleID":"bzflag"}))
	pprint.pprint(badv)
...
badv = json.loads(api('/virtue/create', {"userToken":admintok, "roleID":"bzflag", "userID":username}))
pprint.pprint(badv)

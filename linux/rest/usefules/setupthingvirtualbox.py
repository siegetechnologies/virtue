...
pc('create an app for virtualbox', bc.HEADER)
goodrole = json.loads(api('/application/create', {"userToken":admintok,
	"applicationID":"virtualbox",
	"install":"virtualbox wget unzip",
	"launchCmd":"virtualbox",
	"icon":"https://upload.wikimedia.org/wikipedia/commons/d/d5/Virtualbox_logo.png",
#	"postInstall":"wget https://az792536.vo.msecnd.net/vms/VMBuild_20180425/VirtualBox/MSEdge/MSEdge.Win10.VirtualBox.zip -o /home/docker/win10vbox.zip"
	"type":"linux",
	}))
...
pc('create a role for virtualbox', bc.HEADER)
goodrole = json.loads(api('/role/create', {"userToken":admintok, "roleID":"virtualbox", "applicationIDs":["xterm", "virtualbox"]}))
pprint.pprint(goodrole)

pc('ok, admin will add rolo', bc.HEADER)
badv = json.loads(api('/user/role/authorize', {"userToken":admintok, "username":username, "roleID":"virtualbox"}))
pprint.pprint(badv)
...
while True:
	badv = json.loads(api('/role/get', {"userToken":admintok, "roleID":"virtualbox"}))
	pprint.pprint(badv)
...
badv = json.loads(api('/virtue/create', {"userToken":admintok, "roleID":"virtualbox", "userID":username}))
pprint.pprint(badv)
